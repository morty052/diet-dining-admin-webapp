...
function SetNewPasswordScreen({ isAffiliate }: { isAffiliate?: boolean }) {
  ...
  async function handleConfirmPassword() {
    ...
    // * HANDLE AFFILIATE
    if (isAffiliate) {
      const res = await fetch(`${baseUrl}/affiliates/set-password?password=${password}&affiliate_id=${_id}`)
    ...
    // * HANDLE ADMIN
    if (!isAffiliate) {
      const res = await fetch(`${baseUrl}/admin/set-password?password=${password}&admin_id=${_id}`)
  ...
  return (
    ...
    >
      <div className="">
        <p className="text-2xl font-semibold text-white">Please enter new password</p>
        <p className="text-gray-400">Please Enter a new password for your account. </p>
      </div>
      <div className="">
        <input
          onFocus={() => setError('')}
...
const RegisterCompanionScreen = ({ isAffiliate }: { isAffiliate?: boolean }) => {
  const [error, setError] = React.useState('')
  const navigate = useNavigate()
  ...
  async function handleFinish() {
    if (isAffiliate) {
      ...
      if (status == 'REJECTED') {
        setError('Companion has not been registered please try again after completing steps above')
        return
      }
    }
  ...
  return (
    ...
    >
      <div className="">
        ...
        <p className="text-gray-400">Please follow steps below to register your companion </p>
      </div>
    ...
    return (
      <div className="h-screen">
        <Routes>
          ...
          <Route path="/register-companion" element={<RegisterCompanionScreen isAffiliate />} />
        </Routes>
      </div>
    )
  }
  ...
  return (
    <div className="h-screen">
      <Routes>
        ...
        <Route path="/register-companion" element={<RegisterCompanionScreen />} />
      </Routes>
    </div>
  )
}
