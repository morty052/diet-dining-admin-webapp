...
function ImageInput({ setUploadable }) {
...
function NewProductForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch(`http://localhost:3000/affiliates/add-product?affiliate_id=${_id}`, {
      method: 'POST',
  ...
  async function handleWriteWithAi() {
    const url = `http://localhost:3000/?meal_name=${product?.name}`
    if (writing) {
      return
    }
    setWriting(true)
    ...
    <div className=" pt-10 ">
      <form className=" mx-auto flex max-w-2xl flex-col gap-y-4 ">
        <div className="flex w-full items-center justify-center gap-x-2 md:justify-start">
          <ImageInput setUploadable={setImage} />
