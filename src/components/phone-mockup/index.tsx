...
const mockTags = ['Store', 'tags', 'will', 'appear', 'here', 'when', 'you', 'add', 'them']

export function PhoneMockUp({ synced }: { synced: boolean }) {
  const [viewingStoreInfo, setViewingStoreInfo] = useState(false)

  const { affiliate, previewHeader, previewLogo, removeTag } = useNewAffiliate()
  ...
  return (
    <div className={` mx-auto h-[580px] w-[300px]  rounded-3xl  ${!synced && ''}`}>
      {/* {viewingStoreInfo && (
          <>
          ...
          <div className="h-full rounded-3xl bg-white pt-[196px]">
            <div className="px-2">
              ...
              <div className="flex items-center justify-between">
                <p className="flex-1 text-sm font-medium text-dark">{store_name ? store_name : 'Store Name'}</p>
                <button onClick={() => setViewingStoreInfo(true)} className="rounded-3xl bg-brandGreen px-2 py-1">
                  <p className="text-[10px] font-medium text-white">Store info</p>
                </button>
              </div>
              {/* STORE TAGS */}
        ...
        <div className="relative mx-auto h-[570px] w-[290px] overflow-hidden rounded-3xl  border ">
          ...
          <div className="absolute top-0 h-48 w-full rounded-t-3xl bg-white">
            <div className="relative h-full w-full">
              ...
              {/* HEADER IMAGE */}
              <img className="h-full w-full rounded-t-3xl  object-cover" src={!store_image ? bg : store_image} alt="" />
              {/* HEADER LOGO */}
              {/* <div className="absolute bottom-0 left-0 h-14 w-14 ">
                <img
                  className="h-full w-full rounded-full  object-cover"
